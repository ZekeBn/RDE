...
from insumos_lista
inner join conteo_detalles on conteo_detalles.idinsumo = insumos_lista.idinsumo
where 
insumos_lista.estado = 'A'
and insumos_lista.hab_invent = 1
and conteo_detalles.idconteo = $idconteo
/* and insumos_lista.idgrupoinsu in (SELECT idgrupoinsu FROM conteo_grupos where idconteo = $idconteo) */
$whereadd
order by (SELECT nombre FROM grupo_insumos where idgrupoinsu = insumos_lista.idgrupoinsu) asc, insumos_lista.descripcion asc
";
...
/* and insumos_lista.idgrupoinsu in (SELECT idgrupoinsu FROM conteo_grupos where idconteo = $idconteo) */
$whereadd
order by conteo_detalles.fechahora desc
limit 1
";
...
Ultimo Registrado: <?php if (intval($rs3->fields['idinsumo']) > 0) {
    echo $rs3->fields['descripcion']; ?> [<?php echo $rs3->fields['idinsumo']; ?>] | <?php echo $rs3->fields['codbar']; ?> | <?php echo date("d/m/Y H:i:s", strtotime($rs3->fields['fechahora']));
} ?><br />
<br />
...
<form id="form1" name="form1" method="post" action="conteo_stock_contar.php?id=<?php echo $idconteo; ?>" enctype="application/json">
