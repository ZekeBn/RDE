...
$buscar = "select * from gest_transferencias where idempresa=$idempresa and estado=1 and generado_por=$idusu order by idtanda asc";
...
if (isset($_POST['ter']) && ($_POST['ter']) != '') {
    ...
    $fecha_transferencia = $rsf->fields['fecha_transferencia'];
    ...
    // validar produccion automatica si aplica
    $consulta = "
    SELECT * 
    FROM tmp_transfer
    inner join gest_transferencias on gest_transferencias.idtanda = tmp_transfer.idtanda
    inner join produccion_automatica_traslado on 
    produccion_automatica_traslado.iddeposito_destino = gest_transferencias.destino
    and produccion_automatica_traslado.idinsumo_traslado = tmp_transfer.idproducto 
    where 
    tmp_transfer.idtanda = $idtanda
    and produccion_automatica_traslado.estado = 1
    ";
    //echo $consulta;exit;
    $rsval = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
    // genera una matriz de arrays para luego validar
    $ipa = 1;
    while (!$rsval->EOF) {

        $idreceta = $rsval->fields['idreceta'];
        $consulta = "
        select cantidad_resultante from recetas_produccion where idreceta = $idreceta limit 1
        ";
        $rsrec = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));

        $parametros_arrays[$ipa] = [
            'idreceta' => $rsval->fields['idreceta'],
            'tipo_entrega_insumo' => 1, // siempre 1
            'resultante_real' => floatval($rsrec->fields['cantidad_resultante'] * $rsval->fields['cantidad']),
            'iddeposito' => $rsval->fields['destino'],
            'opr' => 0,
            'idtandatraslado' => $idtanda,
            'prod_auto' => 'S',
    ...
    // recorre la matriz y valida
    foreach ($parametros_arrays as $parametros_array) {
        $res = validar_produccion($parametros_array);
        if ($res['valido'] == 'N') {
            $valido = $res['valido'];
    ...
    reset($parametros_arrays);
    //print_r($parametros_arrays);exit;
    ...
    if ($valido == "S") {
        ...
        while (!$rsf->EOF) {
            ...
            $idinsumo_traslado = $rsf->fields['idproducto'];
            $cantidad_traslado = $rsf->fields['cantidad'];
            $lote = intval($rsf->fields['lote']);
            $vto = antisqlinyeccion($rsf->fields['vto'], 'date');
            $iddeposito_origen = $rsf->fields['origen'];
            ...
            if (intval($rsst->fields['idproducto']) == 0) {
                ...
                $conexion->Execute($insertar) or die(errorpg($conexion, $insertar));
                // no hacer por que es cero movimientos_stock($idinsumo_traslado,0,$iddeposito_origen,4,'+',$tfin,$fecha_transferencia);
            }
            ...
            if (intval($rsst->fields['idproducto']) == 0) {
                ...
                $conexion->Execute($insertar) or die(errorpg($conexion, $insertar));
                // no hacer por que es cero movimientos_stock($idinsumo_traslado,0,$iddeposito_destino,4,'+',$tfin,$fecha_transferencia);
            }
            ...
            movimientos_stock($idinsumo_traslado, $cantidad_traslado, $iddeposito_origen, 3, '-', $tfin, $fecha_transferencia);
            ...
            ";
            $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
            movimientos_stock($idinsumo_traslado, $cantidad_traslado, $iddeposito_destino, 4, '+', $tfin, $fecha_transferencia);

            // inserta en tabla de traslados
            $insertar = " 
            insert into gest_depositos_mov
            (iddeposito,tipomov,deschar,idusu,origen,lote,vto,cantidad,cantidad_recibe,destino,obs,idproducto,idseriecostos,idempresa,idtanda,recibio_destino)
            values
            ($iddeposito_origen,2,'TRASLADO DIRECTO',$idusu,$iddeposito_origen,$lote,$vto,$cantidad_traslado,$cantidad_traslado,$iddeposito_destino,'',$idinsumo_traslado,0,$idempresa,$tfin,'S')
            ";
            $conexion->Execute($insertar) or die(errorpg($conexion, $insertar));
        ...
        //actualizamos el costo (mejorar despues para que la funcion traiga el costeo de las tandas)
        $consulta = "
        update gest_depositos_mov
        set 
        costo_unitario_art = COALESCE((select costo from insumos_lista where idinsumo = gest_depositos_mov.idproducto),0)
        WHERE
        idtanda=$tfin
        ";
        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        ...
        and estado = 1
        and idproducto in (
            select idinsumo_traslado 
            from produccion_automatica_traslado
            where
            iddeposito_destino = gest_depositos_mov.destino
        )
        ";
        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));

        // vuelve el contador al primer registro
        reset($parametros_arrays);
        // recorre y registra la produccion
        foreach ($parametros_arrays as $parametros_array) {

            $res = producir($parametros_array);
            // regisrar el idproducido en table de traslados det
        ...
        header("Location: gest_adm_depositos_mover_tanda.php?l=1");
        exit;
...
function busca_insumo(valor){
    ...
    if(n > 2){
       ...
       $.ajax({
                data:  parametros,
                url:   'gest_mover_stock_cuadro.php',
                type:  'post',
                beforeSend: function () {
...
function busca_insumo_barcode(){
    //alert('Llega');
       var valor = $("#barcode").val();
...
<div class="colcompleto" id="contenedor">
    ...
    <img src="img/pdf.png" width="64" height="64" alt="Descargar PFD" title="Descargar PFD"/>
    </a>
    <a href="gest_transferencias_imp_dir_tk.php?id=<?php echo $tandante; ?>&redir=3" >     
    <img src="img/1495739930_printer.png" width="64" height="64" title="Impresion Ticket" />
    </a>
    <?php } ?>
    </div>
    <div class="divstd">
...
<?php } ?>
    <div class="resumenmini"><h3><br />
      ...
      <form id="comenzart" name="comenzart" action="gest_adm_depositos_mover_tanda.php" method="post">
      
        <table width="200" >
         <?php if ($estado == 1) {?>
        <tr>
           <td colspan="2" align="center"><strong><a href="gest_transferencias_del_directo.php">[Borrar]</a></strong></td>
         </tr>
        <tr>
      ...
      <?php }?>
         <tr>
           <td height="37" align="right" bgcolor="#EBEBEB"><strong>Tanda:</strong></td>
           <td align="left"><?php echo $idtanda; ?></td>
         </tr>
         <tr>
    ...
    <form id="fin" name="fin" action="gest_adm_depositos_mover_tanda.php" method="post">
    ...
    <div id="productos" align="center">
        ...
        <table width="600">
             <tr>
                <td>
                    <input type="text" name="barcode" id="barcode" style="width:99%; height:40px;" value="<?php echo htmlentities($_POST['barcode']);?>" placeholder="Cod. Barras" onkeyup="busca_insumo_barcode();"  />
               
               </td>
               <td>
                    <input type="text" name="codigoprod" id="codigoprod" style="width:99%; height:40px;" value="<?php echo htmlentities($_POST['codigoprod']);?>" placeholder="codigo"  />
               
               </td>
                <td height="28" colspan="2">
                    <input type="text" name="codigop" id="codigop" style="width:99%; height:40px;" value="<?php if ($_POST['codigoprod'] == '') {
        ...
        while (!$gr->EOF) {?>
                ...
                <td width="26">
                    <a href="javascript:void(0);" onclick="busca_insumo_cod();"><img src="img/buscar.png" width="32" height="32" alt=""/></a>
