...
if (($pedido > 0 or $idmesa > 0)) {
    ...
    //cuerpo
    //$campos = traer_campos("tmp_ventares","idventatmp");
    if ($idmesa == 0) {
        $consulta_det_ped = "
		SELECT * ,
		$whereadd_depo
        ...
        while (!$rsdet->EOF) {
            ...
            if ($idreceta > 0) {
                ...
                $consulta = "
					...
					";
                $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                //faltaba el descuento de stock del producto////

                $consulta = "
					select mueve_stock,idinsumo from insumos_lista where idproducto = $idprod
				";
                $rsprord = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                $mueve_stock_prod = $rsprord->fields['mueve_stock'];
                $idinsumo = $rsprord->fields['idinsumo'];
                // descuenta stock general
                if ($mueve_stock != 'N' && $mueve_stock_prod != 'N') {
                    descontar_stock_general($idinsumo, 1, $iddeposito_det);
                    movimientos_stock($idinsumo, 1, $iddeposito_det, 2, '-', $idventa, $ahora);
                }
            }
            // si es un combinado
            if ($combinado_es == 'S') {
            ...
            if ($idreceta == 0 && $combinado_es == 'N' && $combo_es == 'N') {
                //faltaba el descuento de stock del producto////
                $consulta = "
					select mueve_stock,idinsumo from insumos_lista where idproducto = $idprod
				";
