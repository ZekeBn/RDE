...
function relacionar_gastos($parametros_array)
{
    ...
    global $preferencias_facturas_multiples;
    global $proveedores_importacion;
    global $ids_no_aplica;  // Asegúrate de que esta variable sea global

    // Inicializa $ids_no_aplica como un array vacío si no está definido
    if (!isset($ids_no_aplica) || !is_array($ids_no_aplica)) {
        $ids_no_aplica = [];
    }
    ...
    if (intval($idcompra_ref) > 0) {
        ...
        $consulta = "SELECT idcompra FROM compras WHERE idcompra_ref=$idcompra_ref AND estado != 6";
        ...
        foreach ($rs_id_array_gastos as $key => $value) {
            ...
            $consulta = "SELECT compras_detalles.subtotal AS gastos, compras.iva5, compras.iva10, insumos_lista.descripcion, facturas_proveedores_det_impuesto.ivaml, facturas_proveedores_det_impuesto.gravadoml
            FROM compras_detalles
        ...
        $consulta = "SELECT SUM(subtotal) AS total FROM compras_detalles WHERE idcompra=$idcompra_ref
        AND compras_detalles.codprod NOT IN (
            SELECT idinsumo FROM insumos_lista 
            WHERE UPPER(insumos_lista.descripcion) LIKE \"%DESCUENTO%'\" 
            OR UPPER(insumos_lista.descripcion) LIKE \"%AJUSTE%\"
        ...
        while (!$rs_detalles_compras->EOF) {
            ...
            $consulta = "UPDATE compras_detalles SET gastos=$gastos WHERE idregs = $idregs";
            $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
            ...
            if (!in_array($idp, $ids_no_aplica)) {
                if ($maneja_lote == 1) {
                    $update = "UPDATE costo_productos SET costo_promedio=$precio_costo, modificado_el='$ahora', precio_costo=$costo WHERE id_producto=$idp AND idcompra=$idcompra_ref $whereadd";
                    $conexion->Execute($update) or die(errorpg($conexion, $update));
                }

                if ($preferencias_costo_promedio == "S") {
                    $consulta = "UPDATE insumos_lista SET costo = COALESCE((SELECT costo_promedio FROM costo_productos WHERE idseriepkcos = $idseriepkcos),0) WHERE idinsumo = $codprod AND idempresa = $idempresa";
                    $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                }
            }
