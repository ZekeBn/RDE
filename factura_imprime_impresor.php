...
require_once("test_imprime_fac_app.php");
...
$pie_pagina = $rsimprem->fields['pie_pagina'];
$metodo_app = $rsimprem->fields['metodo_app'];
$defaultprnt = "http://localhost/impresorweb/ladocliente.php";
$script_impresora_rem = trim($rsimprem->fields['script']);
$version_app = $rsimprem->fields['version_app'];
$tipo_saltolinea_app = $rsimprem->fields['tipo_saltolinea_app'];
...
// auto impresor para app FACTURA
//$factura_auto_app=str_replace($saltolinea,'\\r ',$factura_auto);
// ticket para app
...
// auto impresor para app TICKET
// ticket para app
if ($tipo_saltolinea_app != '') {
    $ticket_app = str_replace($saltolinea, $tipo_saltolinea_app, $texto); // \\r
} else {
    $ticket_app = $texto;
}
$ticket_app = str_replace("'", "", $ticket_app);
$ticket_app = str_replace('"', '', $ticket_app);
$texto_tk_app = $ticket_app;
...
//parametros para la funcion
$parametros_array_tk = [
    'texto_imprime' => $texto_tk_app, // texto a imprimir
    'url_redir' => $url1, // redireccion luego de imprimir
    'lista_post' => $lista_post, // se usa solo con metodo POST_URL
    'imp_url' => $script_impresora_rem, // se usa solo con metodo POST_URL
    'metodo' => $metodo_app // POST_URL, SUNMI, ''
...
if ($tk == 1) {
    ...
    if ($moduloventa == 1) {
        ...
        if ($rspref->fields['imprime_alfinalizar'] == 'S') {
            ...
            echo "    document.location.href='".$url1."';".$saltolinea;
        }
