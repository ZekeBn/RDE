...
//echo json_encode($_GET);echo json_encode($_POST);exit;
if ($ocnum == 0) {
...
$whereadd = "";
if ($insumos_proveedor == "S" && $ocnum > 0) {
    $buscar = "SELECT idproveedor from compras_ordenes where ocnum = $ocnum";
    $rs_ocnum = $conexion->Execute($buscar) or die(errorpg($conexion, $buscar));
    $idproveedor = $rs_ocnum->fields['idproveedor'];
    $whereadd = " AND idproveedor = $idproveedor";
}
...
<div class="divizquierda350">
    <select name="lprod" size="4" id="lprod" style="width:100%;">
        ...
        ?>
            <option <?php echo $clase;  ?>
                onclick="verificar_insumo(<?php echo $rsp->fields['idinsumo']; ?>)"
                data-hidden-idmedida="<?php echo antixss($rsp->fields['idmedida']); ?>"
                data-hidden-medida="<?php echo antixss($rsp->fields['medida']); ?>"
                data-hidden-idmedida2="<?php echo antixss($rsp->fields['idmedida2']); ?>"
                data-hidden-medida2="<?php echo antixss($rsp->fields['medida2']); ?>"
                data-hidden-cant-medida2="<?php echo intval($rsp->fields['cant_medida2']); ?>"
                data-hidden-idmedida3="<?php echo antixss($rsp->fields['idmedida3']); ?>"
                data-hidden-medida3="<?php echo antixss($rsp->fields['medida3']); ?>"
                data-hidden-cant-medida3="<?php echo intval($rsp->fields['cant_medida3']); ?>"
                data-hidden-cant-edi="<?php echo intval($rsp->fields['cant_caja_edi']); ?>"
                data-hidden-id-edi="<?php echo intval($id_cajas_edi); ?>"
                value="<?php echo $rsp->fields['idinsumo'] ?>"
                <?php if ($tprod == 1) { ?>selected="selected" <?php } ?>>
                <?php echo $rsp->fields['descripcion'] ?> [<?php echo $rsp->fields['idinsumo']; ?>] - <?php echo $rsp->fields['barcode'] ?>
            </option>
        <?php
            $i++;
            $rsp->MoveNext();
