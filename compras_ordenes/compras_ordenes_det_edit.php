...
$costo_cant_medida2 = ($precio_compra_total / $cantidad_original) * $cant_medida2;
//echo $precio_compra_total, "--",$cantidad_original,"--", $cant_medida2, "--" ,$costo_cant_medida2;exit;
...
if (isset($_POST['MM_update']) && $_POST['MM_update'] == 'form1') {
    ...
    $precioCaja = antisqlinyeccion($_POST['precio_caja'], "float");
    ...
    // echo 'precio total 1' . $precio_total;
    //echo $cant_ref, "--", $precio_compra; exit;
    //$precio_total = $cantidad * $precio;
    //echo 'precio total 2' . $precio_total;
    ...
    //echo $precio_compra,"---",$precioCaja; exit;
...
<script>
    function cargarMedidaEdit() {
        ...
        if (cant_medida2 != undefined && cant_unidad % cant_medida2 == 0) {
            var result_bulto = (cant_unidad / cant_medida2 * 100).toFixed(0) / 100;
            $("#form1Edit #bulto").val(result_bulto);
        }

        if (cant_medida3 != undefined && (cant_unidad / cant_medida2) % cant_medida3 > 0) {
            var result_pallet = ((cant_unidad / cant_medida2) / cant_medida3 * 100).toFixed(0) / 100;
            $("#form1Edit #pallet").val(result_pallet);
        }
    }
    ...
    function cargarMedida2Edit(value, limpiar) {
        ...
        // fue excluido el calculo del resto porque al ser multiplo del dato que tiene el articulo no actualiza el campo en el formulario
    ...
    // Función para actualizar el valor de precio_compra
    function actualizarPrecioCompra() {
        var precioCaja = parseFloat(document.getElementById("precio_caja").value);
        var cantMedida2 = <?php echo $cant_medida2; ?>; // Obtener el valor de $cant_medida2 desde PHP
        var nuevoPrecioCompra = precioCaja / cantMedida2;
        document.getElementById("precio_compra").value = nuevoPrecioCompra.toFixed(2);
    }

    // Función para actualizar el valor de precio_caja
    function actualizarPrecioCaja() {
        var precioCompra = parseFloat(document.getElementById("precio_compra").value);
        var cantMedida2 = <?php echo $cant_medida2; ?>; // Obtener el valor de $cant_medida2 desde PHP
        var nuevoPrecioCaja = precioCompra * cantMedida2;
        document.getElementById("precio_caja").value = nuevoPrecioCaja.toFixed(2);
    }

    // Evento onchange para detectar cambios en precio_caja y llamar a la función actualizarPrecioCompra
    document.getElementById("precio_caja").onchange = function() {
        actualizarPrecioCompra();
    };

    // Evento onchange para detectar cambios en precio_compra y llamar a la función actualizarPrecioCaja
    document.getElementById("precio_compra").onchange = function() {
        actualizarPrecioCaja();
    };

    // Llamar a las funciones actualizarPrecioCompra y actualizarPrecioCaja al cargar la página para inicializar los valores de precio_compra y precio_caja
...
<form id="form1Edit" name="form1Edit" method="post" action="">
    ...
    <div class="form-group col-md-6 col-xs-12">
        <label class="control-label col-md-3 col-sm-3 col-xs-12">Precio Caja *</label>
        <div class="col-md-9 col-sm-9 col-xs-12">
            <input type="text" name="precio_caja" id="precio_caja" value="<?php if (isset($_POST['precio_caja'])) {
                                                                                echo floatval($_POST['precio_caja']);
                                                                            } else {
                                                                                echo floatval($costo_cant_medida2);
                                                                            } ?>" placeholder="Precio compra unitario" class="form-control" />
            <span style="color: red; font-weight: bold; font-family: 'Arial', sans-serif;">Precio de compra por caja</span>
        </div>
    </div>
