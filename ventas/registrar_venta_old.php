...
// si es autoimpresor, si o si es factura
if ($autoimpresor == 'S') {
...
if ($generatmpcab == 'S') {
    ...
    if ($valido == 'S') {
		...
		VALUES 
		('$razon_social', '$ruc', $chapa, $observacion, $montototal, $idusu, '$ahora', $idsucursal, $idempresa,$canal,$delivery,$idmesa,1,$tipoventa,$idzona,$delivery_costo,$nombre_deliv,$apellido_deliv,$idclientedel,$iddomicilio)
		";
        //echo $consulta;
        //exit;
...
if (($pedido > 0 or $idmesa > 0)) {
    ...
    if ($valido == 'S') {
        ...
        $insertar = "Insert into ventas
			...
			idtandatimbrado,factura_sucursal,factura_puntoexpedicion,timbrado)
			VALUES
			('$ahora',$idcliente,$tipoventa,$idempresa,$idsucursal,$factura,NULL,$ruc_pri,$ruc_dv,$totalvendido,0,$tcid,$idusu,$iva10,$iva5,$tventaex
			...
			$idtandatimbrado,$factura_suc,$factura_pexp,$timbrado
        ...
        if ($tipoventa == 2) {
            ...
            $insertar = "
				...
				(idempresa,sucursal,deuda_global,saldo_activo,idcliente,estado,registrado_el,registrado_por,idventa,idadherente,idserviciocom)
				values
				($idempresa,$idsucursal,$totalcobrar,$totalcobrar,$idcliente,1,'$ahora',$idusu,$idventa,$idadherente,$idservcom)
				";
            $conexion->Execute($insertar) or die(errorpg($conexion, $insertar));

            //Credito
            $buscar = "Select max(idcta) as mayor from cuentas_clientes where idempresa=$idempresa and registrado_por = $idusu";
        ...
        while (!$rsdet->EOF) {
            ...
            if ($tag > 0){
                while (!$ag->EOF){
                    ...
                    insumos_lista.mueve_stock as mueve_stock_ins, insumos_lista.idproducto,
                    (select tipoiva from productos where idprod_serial = insumos_lista.idproducto) as iva
                    from insumos_lista
                    ...
                    $mueve_stock_ins=trim($rsinsu->fields['mueve_stock_ins']);
                    $idproducto_ag=intval($rsinsu->fields['idproducto']);
                    $iva=intval($rsinsu->fields['iva']);
                    ...
                    if($mueve_stock != 'N' && $mueve_stock_ins != 'N'){
                        ...
                        // costo unitario promedio de los insumos utilizados
                        if($canti > 0){
                            $costo_insumo_unitario=round($costo_insumo_utilizado/$canti,4);
                        }else{
                            $costo_insumo_unitario=0;
                        }
                        // costo del insumo actual utilizado
                    ...
                    if(!($costo_insumo_utilizado > 0)){
                        $costo_insumo_utilizado=0;
                    }
                    // insertar en venta receta VIEJO		NO USAR MAS
                    //$insertar="Insert into venta_receta
                    //(idventa,idventadet,idprod,idingrediente,idinsumo,cantidad,idmedida,costo,fechahora)
                    //values
                    ...
                    (pventa, cantidad, subtotal, idventa, idemp, sucursal, idprod, pchar, retirado_por, costo, subtotal_costo, utilidad, iva, registrado_el, descuento, subtotal_sindesc, descuento_porc, subtotal_monto_iva, idprod_mitad1, idprod_mitad2, existencia, serialcostos, subtotal_monto_iva_excluido_diplomatico, iva_excluido_diplomatico, subtotal_anterior_diplomatico, pventa_anterior_diplomatico, pventa_monto_iva_anterior, cortesia, rechazo, idmotivorecha)
                    values
                    ($pre, 1, $pre, $idventa, $idempresa, $idsucursal, $idproducto_ag, NULL, NULL, 0, 0, 0, $iva, '$ahora', 0, 0, 0, 0, NULL, NULL, 0, 0, 0, 0, 0, 0, 0, NULL, NULL, NULL)
                    ";
                    $conexion->Execute($consulta) or die(errorpg($conexion,$consulta));

                    $consulta="select max(idventadet) as idventadet_ag from ventas_detalles";
                    $rs=$conexion->Execute($consulta) or die(errorpg($conexion,$consulta));
                    $idventadet_ag=intval($rs->fields['idventadet_ag']);

                    // insertar en venta receta
                    $consulta="Insert into venta_receta
                    (idventa,idventadet,idprod,idingrediente,idinsumo,cantidad,idmedida,costo,costo_unitario,fechahora)
                    values
                    ($idventa,$idventadet_ag,$idproducto_ag,$ingediente,$insu,$canti,$idmedida,$costo_insumo_utilizado,$costo_insumo_unitario,'$ahora')";
                    $conexion->Execute($consulta) or die(errorpg($conexion,$consulta));
        ...
        if (intval($fac_nro) > 0 && $autoimpresor != 'S') {
            $numfac = intval($fac_nro);
            $ano = date("Y");
            // busca si existe algun registro
            $buscar = "Select max(numfac) as mayor from lastcomprobantes where idsuc=$fac_suc and pe=$fac_pexp and idempresa=$idempresa ";
