...
if ($ventas_nostock == 2) {
    ...
    if ($generatmpcab == 'S') {
        ...
        if ($pedido > 0) {
            $buscar = "
			Select tmp_ventares.idproducto, productos.descripcion
			from  tmp_ventares_cab
		...
		and (select insumos_lista.hab_invent from insumos_lista where insumos_lista.idproducto = tmp_ventares.idproducto)  = 1
            ...
            $buscar = "Select tmp_ventares.idproducto, productos.descripcion
			from  tmp_ventares_cab 
			inner join tmp_ventares on tmp_ventares.idtmpventares_cab = tmp_ventares_cab.idtmpventares_cab
			inner join productos on tmp_ventares.idproducto = productos.idprod_serial
			where  
...
if ($generatmpcab == 'S') {
    ...
    if ($valido == 'S') {
		...
		VALUES 
		('$razon_social', '$ruc', $chapa, $observacion, $montototal, $idusu, '$ahora', $idsucursal, $idempresa,$canal,$delivery,$idmesa,1,$tipoventa,$idzona,$delivery_costo,$nombre_deliv,$apellido_deliv,$idclientedel,$iddomicilio,$idmotorista,$idcanalventa,$direccion,$telefono)
		";
        //echo $consulta;
        //exit;
        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
...
if (($pedido > 0 or $idmesa > 0)) {
    ...
    if ($valido == 'S') {
        ...
        $insertar = "Insert into ventas
			...
			idsucursal_clie,idapp, idsalon_asig)
			VALUES
			...
			$diplomatico,$carnet_diplomatico,$idmotorista,$idcanalventa,$codpedido_externo,
			$idsucursal_clie,$idapp, $idsalon_usu
        ...
        while (!$rsdet->EOF) {
            ...
            $insertar = "Insert into ventas_detalles 
				...
				base_imponible,idtipoiva,subtotal_monto_iva,pchar,idlistaprecio)
				values
        ...
        if ($descuento_sobre_factura > 0 && $idproducto_descuento > 0) {
            ...
            $consulta = "
				...
				VALUES 
				(
				$descuento_negativo, 1, $descuento_negativo, $idventa, 1, $idsucursal, $idproducto_descuento, NULL,
