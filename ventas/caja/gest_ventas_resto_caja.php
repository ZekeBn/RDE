...
select *, $seladd_lp,
	...
	(
		...
		from insumos_lista
		left join gest_depositos_stock_gral on insumos_lista.idinsumo = gest_depositos_stock_gral.idproducto 
		and gest_depositos_stock_gral.iddeposito in (
						SELECT iddeposito
						FROM gest_depositos
						where
						tiposala = 2
		...
		where
		insumos_lista.idproducto = productos.idprod_serial
...
<?php
        ...
        while (!$rsprod->EOF) {
            $cantidad_restante = floatval($rsprod->fields['stock_teorico']) - floatval($rsprod->fields['total']);
            $img = "../../gfx/productos/prod_".$rsprod->fields['idprod_serial'].".jpg";
            if (!file_exists($img)) {
                $img = "../../gfx/productos/prod_0.jpg";
            }

            ?><div id="prod_<?php echo $rsprod->fields['idprod_serial']; ?>" class="producto <?php if ($cantidad_restante <= 0 && $rsprod->fields['venta_sin_stock'] != 1) {?> hide <?php } ?>" <?php
if ($rsprod->fields['idtipoproducto'] == 1) { // producto
    ...
    <?php if (trim($rsprod->fields['descripcion']) != '') { ?><img src="<?php echo $img ?>" height="81" width="163" border="0" alt="<?php echo Capitalizar(trim($rsprod->fields['descripcion'])); ?>" title="<?php echo Capitalizar(trim($rsprod->fields['descripcion'])); ?>" /><br /><?php echo Capitalizar(trim($rsprod->fields['descripcion'])); ?><br />Gs. <?php if (trim($rsprod->fields['combinado']) == 'N') { ?><?php echo formatomoneda(trim($rsprod->fields['p1'])); ?><?php } else {?>Precio Variable<?php } ?><br>En Stock: <div id="stock_cantidad_<?php echo $rsprod->fields['idprod_serial']; ?>" style="display: inline;" ><?php echo $cantidad_restante; ?></div><div <?php if (($rsprod->fields['venta_sin_stock'] != 1)) {?> data-hidden-value="false" <?php } else { ?> data-hidden-value="true" <?php } ?> id="hidden_stock_cantidad_<?php echo $rsprod->fields['idprod_serial']; ?>" style="display: none;" ><?php echo $cantidad_restante; ?></div><input type="hidden" value="<?php echo $rsprod->fields['p1']; ?>" name="precio_<?php echo $rsprod->fields['idprod_serial']; ?>" id="precio_<?php echo $rsprod->fields['idprod_serial']; ?>" >
    <br /><?php } ?>
</div>
    <?php $x++;
            $rsprod->MoveNext();
...
<script src="../../js/ventas_caja.js"></script>
