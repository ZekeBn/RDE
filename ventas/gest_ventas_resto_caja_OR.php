    ...
    if ($cat > 0) {
        ...
        if (strtoupper($rsfav->fields['tipofavorito']) == 'F') {
			...
			and idprod_serial in (
						select idprod_serial from 
							(
							select p.idprod_serial
							from ventas 
							...
							order by count(ventas_detalles.idprod) desc
							limit $maxprod
							) as topprod
					)
			 ";
       ...
       $.ajax({
                ...
                success:  function (response) {
					...
					if(IsJsonString(response)){
						...
						if(obj.error == ''){
							if(obj.redirfac == 'S'){
							   document.body.innerHTML='<meta http-equiv="refresh" content="0; url=<?php echo $script?>?v='+obj.idventa+'<?php echo $redirbus2; ?>">';
							 }else{
								 document.body.innerHTML='<meta http-equiv="refresh" content="0; url=gest_ventas_resto_caja.php<?php echo $redirbus; ?>">';
							 }
						}else{
							alertar_redir('NO SE REGISTRO LA VENTA',obj.error,'error','ACEPTAR','gest_ventas_resto_caja.php<?php echo $redirbus; ?>');
						}
					}else{
						alert(response);
					}
