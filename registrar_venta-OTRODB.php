...
/*
ventas afectadas bellini
select * from ventas where idventa in ( SELECT idventa FROM `ventas_detalles` WHERE `idprod` = 264 and date(registrado_el) >= '2024-04-01' );

*/

//print_r($_POST);exit;
//echo $_POST['parametros_pos'];


//print_r($_POST['parametros_pos']);
//exit;


//$parametros = json_decode($_POST['parametros_pos'], true);
//echo $parametros;exit;


//echo json_encode($_POST['parametros_pos'], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);exit;
//$parametros_pos=json_encode($_POST['parametros_pos']);
//print_r($parametros_pos);exit;
...
// preferencias caja
$consulta = "
SELECT  usa_tarjetadelivery,
usa_motorista, obliga_motorista, valida_duplic_tipo,
 obliga_cod_transfer, usa_orden_compra, obliga_orden_compra, 
 redondeo_pref, ceros_redondeo, direccion_redondeo, prorratear_descuento
FROM preferencias_caja 
WHERE  
idempresa = $idempresa ";
$rsprefcaj = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
$prorratear_descuento = trim($rsprefcaj->fields['prorratear_descuento']);
if ($facturador_electronico == 'S') {
    $prorratear_descuento = "S";
}
$usa_motorista = trim($rsprefcaj->fields['usa_motorista']);

$usa_tarjetadelivery = trim($rsprefcaj->fields['usa_tarjetadelivery']);
...
// si no es efectivo, no hace redondeo sedeco
...
// canal debe estar primero para  el carrito
...
$idcanal = $canal;
// si no es mesa (modulo viejo)
$where_add_sinmesa = "";
if ($idcanal <> 4) {
    $where_add_sinmesa = " and tmp_ventares.idmesa = 0 ";
}
...
$iddarkkitchen = antisqlinyeccion($_SESSION['iddarkkitchen'], 'int');
$fecha_venta = trim($_POST['fecha_venta']);
$codpedido_externo = antisqlinyeccion($_POST['codpedido_externo'], 'text');
$idconfirmapago = intval($_POST['idconfirmapago']);
$idpostmp = intval($_POST['idpostmp']);
if ($idpostmp > 0) {
    $consulta = "
	select idforma from formas_pago where sin_formapago_pos = 'S' order by idforma asc limit 1
	";
    $rssinforma = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
    $idformapago_pos_sinidentificar = intval($rssinforma->fields['idforma']);
    if ($idformapago_pos_sinidentificar == 0) {
        $valido = 'N';
        $errores .= '- No se indico la forma de pago que se utilizara para issuerId de bancard no identificados.<br />';
    }
}
...
$idmotivodescuento = antisqlinyeccion($_POST['idmotivodescuento'], 'int');
...
if (intval($_POST['cantidad_vale']) > 0) {
    $cantidad_vale = antisqlinyeccion($_POST['cantidad_vale'], 'int');
} else {
    $cantidad_vale = antisqlinyeccion('', 'int');
}
...
if ($ventas_nostock == 2) {
    ...
    if ($generatmpcab == 'S') {
		...
		select * from 
        (
            select tmp_ventares.idproducto, productos.descripcion, sum(COALESCE(tmp_ventares.cantidad,0)) as cantidad_carrito,
                COALESCE((
                 select COALESCE(gest_depositos_stock_gral.disponible,0) as disponible
                from insumos_lista
    		...
    		and tmp_ventares.finalizado = 'N'
    		and tmp_ventares.idsucursal = $idsucursal
    		and (select insumos_lista.hab_invent from insumos_lista where insumos_lista.idproducto = tmp_ventares.idproducto)  = 1 $where_add_sinmesa
        ...
        if (intval($rs->fields['idproducto']) > 0) {
            ...
            $prodsinstock = "";
            while (!$rs->EOF) {
                $stock_car = formatomoneda(floatval($rs->fields['cantidad_carrito']), 4, 'N');
                $stock_dep = formatomoneda(floatval($rs->fields['cantidad_stock']), 4, 'N');
...
if ($generatmpcab == 'S') {
    ...
    if ($valido == 'S') {
		...
		clase,tipoventa,delivery_zona,delivery_costo,nombre_deliv,apellido_deliv,idclientedel,iddomicilio,idmotoristaped,
		idcanalventa,direccion,telefono,idterminal,iddarkkitchen) 
		VALUES 
		('$razon_social', '$ruc', $chapa, $observacion, $montototal, $idusu, '$ahora', $idsucursal, $idempresa,$canal,$delivery,$idmesa,
		1,$tipoventa,$idzona,$delivery_costo,$nombre_deliv,$apellido_deliv,$idclientedel,$iddomicilio,$idmotorista,
		$idcanalventa,$direccion,$telefono,$idterminal_usu,$iddarkkitchen)
		";
        //echo $consulta;
        //exit;
        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        ...
        $conexion->Execute($consulta) or die(errorpg($conexion,$consulta));*/
        $consulta = "
		SELECT GROUP_CONCAT(idventatmp SEPARATOR ',') AS idventatmp_csv
		FROM tmp_ventares
		where
		usuario = $idusu
		and registrado = 'N'
		...
		$where_add_sinmesa
		";
        $rsdetped = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        $idventatmp_csv = trim($rsdetped->fields['idventatmp_csv']);
        $consulta = "
		update tmp_ventares
		set 
		finalizado = 'S',
        ...
        // si es envio pos POS de Bancard
        if ($idpostmp > 0 && $idconfirmapago == 0) {
            $consulta = "
			select * 
			from pos_tmp
			where
            ...
            $tipo = trim($rs->fields['tipo']);
            $parametros_pos = trim($_POST['parametros_pos']);
            $parametros_pos = substr($parametros_pos, 1, -1); // eliminar primer y ultimo caracter ''
            $parametros_pos = json_decode($parametros_pos, true);
            //print_r($parametros_pos);exit;
            $codigoAutorizacion = antisqlinyeccion($parametros_pos['codigoAutorizacion'], "textbox");
            $codigoComercio = antisqlinyeccion($parametros_pos['codigoComercio'], "textbox");
            $issuerId = antisqlinyeccion($parametros_pos['issuerId'], "textbox");
            $mensajeDisplay = antisqlinyeccion(limpia_noimprimibles($parametros_pos['mensajeDisplay']), "textbox");
            $montoVuelto = antisqlinyeccion($parametros_pos['montoVuelto'], "textbox");
            $nombreCliente = antisqlinyeccion(limpia_noimprimibles($parametros_pos['nombreCliente']), "textbox");
            $nombreTarjeta = antisqlinyeccion(limpia_noimprimibles($parametros_pos['nombreTarjeta']), "textbox");
            $nroBoleta = antisqlinyeccion($parametros_pos['nroBoleta'], "textbox");
            $pan = antisqlinyeccion($parametros_pos['pan'], "textbox");
            $saldo = antisqlinyeccion($parametros_pos['saldo'], "textbox");

            /*{"codigoAutorizacion":"562624","codigoComercio":"3057429        ",
                "issuerId":"VD","mensajeDisplay":"APROBADA","montoVuelto":0,
                "nombreCliente":"TARJETA DE/PRUEBA","nombreTarjeta":"BANCARD GOURMET CARD EMV                ",
                "nroBoleta":"003050632337","pan":"4675","saldo":1}*/

            $consulta = "
			INSERT INTO `tmp_ventares_cab_pos`
			(
			`idtmpventares_cab`, `tipo`, `codigoAutorizacion`, `codigoComercio`, `issuerId`, `mensajeDisplay`,
			 `montoVuelto`, `nombreCliente`, `nombreTarjeta`, `nroBoleta`, `pan`, `saldo`
			 ) 
			VALUES
			 ($idtmpventares_cab,'$tipo',$codigoAutorizacion, $codigoComercio, $issuerId, $mensajeDisplay,
			 $montoVuelto, $nombreCliente, $nombreTarjeta, $nroBoleta, $pan, $saldo)
...
if (($pedido > 0 or $idmesa > 0)) {
    ...
    $iva5 = 0;
    $tventaex = 0;
    $iddarkkitchen = antisqlinyeccion($rscabpri->fields['iddarkkitchen'], 'int');
    ...
    if ($ventas_nostock == 2) {
        ...
        /*
        da problemas con productos tipo combo
        $consulta="
        select idproducto, cantidad_carrito, disponible
        from
        (
        select
            pedido.idproducto,
            sum(pedido.cantidad) as cantidad_carrito,
            COALESCE((
                select gest_depositos_stock_gral.disponible
                from gest_depositos_stock_gral
                inner join insumos_lista on insumos_lista.idinsumo=gest_depositos_stock_gral.idproducto
                WHERE
                insumos_lista.idproducto = pedido.idproducto
                and gest_depositos_stock_gral.iddeposito = $iddeposito
                and insumos_lista.idproducto is not null
            ...
            group by pedido.idproducto
        ) as stock_ctr
        where
        cantidad_carrito > disponible;
        ";
        $rsexctrl=$conexion->Execute($consulta) or die(errorpg($conexion,$consulta));
        if(intval($rsexctrl->fields['idproducto']) > 0){
            $valido='N';
            $errores.="- Existen productos [".intval($rsexctrl->fields['idproducto'])."] que superan el stock disponible, no se puede procesar esta venta.";
        }*/
    ...
    if ($valido == 'S') {
        ...
        if ($prorratear_descuento == 'S') {
            if ($descuento > 0) {
                ...
                //echo 'subtotal_new: '.$subtotal_new_acum.' | subtotal con descuento : '.$total_descontado;
                ...
                if ($subtotal_new_acum < $total_descontado) {
                    ...
                    $consulta = "
						update tmp_ventares set subtotal=$subtotal_ajustado where idventatmp = $idventatmp_mascaro
						";
                    //echo $consulta;
                    $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                }
        ...
        $insertar = "Insert into ventas
			...
			total_venta,idtransaccion,trackid,registrado_por,totaliva10,totaliva5,texe,
			idpedido,otrosgs,descneto,deliv,totalcobrar,tipoimpresion,vendedor,total_cobrado,
			estado,obs,idmesa,idcanal,idcaja,idzona,operador_pedido,formapago,vuelto,recibido,
			idadherente,idserviciocom,idclientedel,iddomicilio,
			idtandatimbrado,factura_sucursal,factura_puntoexpedicion,timbrado,ruc,iddeposito,
			diplomatico,carnet_diplomatico,idmotorista,idcanalventa,iddarkkitchen,codpedido_externo,
			idsucursal_clie,idapp, idsalon_asig,ocnumero,
			id_indicador_presencia, idtipotranset, idtipooperacionset, idtipotran, idmotivodescuento, cantidad_vale
			)
			VALUES
			('$fecha_venta',$idcliente,$tipoventa,$idempresa,$idsucursal,$factura,NULL,$ruc_pri,$ruc_dv,
			$totalvendido,0,$tcid,$idusu,$iva10,$iva5,$tventaex,
			$pedido,$delivery_costo,$descuento,'',$totalcobrar,1,$idvendedor,$totalcobrar,
			$esta,$motivo_descuento,$idmesa,$idcanal, 
			$idcaja,$idzona,$operador_pedido,$mediopago,$vuelto,$monto_recibido,
			$idadherente,$idservcom,$idclientedel,$iddomicilio,
        ...
        if ($usa_tarjetadelivery == 'S') {

            $buscar = "Select idtarjetadelivery from tmp_ventares_cab where idtmpventares_cab=$pedido";
            $rfg2 = $conexion->Execute($buscar) or die(errorpg($conexion, $buscar));
            $idtarjeta_delivery = intval($rfg2->fields['idtarjetadelivery']);
            if (($idtarjeta_delivery) > 0) {
                //actualizamos laa tarjeta ocupada como facturado
                $update = "update tarjeta_delivery_movimientos set idventa=$idventa where idtarjetadelivery=$idtarjeta_delivery and estado=1 and idpedido=$pedido";
                $conexion->Execute($update) or die(errorpg($conexion, $update));

                $update = "update tarjeta_delivery set idestadotarjetadel=3, idpedido_ult=NULL where idtarjetadelivery=$idtarjeta_delivery and idestadotarjetadel=2 ";
                $conexion->Execute($update) or die(errorpg($conexion, $update));
            }
        }
        ...
        if ($tipoventa == 1) {
            ...
            if ($mediopago == 3) {
                ...
                while (!$rscarcobven->EOF) {
                    ...
                    $idformapago = $rscarcobven->fields['idformapago'];
                    $idformapago_set = $rscarcobven->fields['idformapago_set'];
                    $voucher_nro = antisqlinyeccion($rscarcobven->fields['voucher_nro'], "text");
                    $consulta = "
                    ...
                    // voucher
                    if (trim($rscarcobven->fields['voucher_nro']) != '') {
                        $insertar = "
							insert into ventas_reg_adicionales
							(idventa,monto_abonado,valor_adicional,registrado_el,registrado_por,estado,idpagodet)
							values
							($idventa,$monto_pago_det,$voucher_nro,'$ahora',$idusu,1,$idpagodet)
							";
                        $conexion->Execute($insertar) or die(errorpg($conexion, $insertar));
                    }
                ...
                if ($adicional_int > 0 && $idventa > 0) {
                    $insertar = "
						...
						(idventa,monto_abonado,valor_adicional,registrado_el,registrado_por,estado,idpagodet)
						values
						($idventa,$totalcobrar,$adicional_int,'$ahora',$idusu,1,$idpagodet)
						";
                    $conexion->Execute($insertar) or die(errorpg($conexion, $insertar));
                }
            }
        ...
        while (!$rsdet->EOF) {
            ...
            $subtotal_orig = floatval($rsdet->fields['subtotal_orig']);
            if ($subtotal_orig > 0) {
                $descuento_prorrateado = $subtotal_orig - $subtotal;
            } else {
            ...
            if ($dip != 'S') {
                ...
                $tipoiva = 0;
                $idtipoiva = 3;
                $id_forma_afectacion_tributaria_iva = 3; // 2 exonerado tabla forma_afectacion_tributaria_iva
            }

            if ($idtipoproducto == 7) {
                $pchar = antisqlinyeccion(substr($rsdet->fields['observacion'], 0, 250), "text");
            } else {
                $pchar = "NULL";
            }
            ...
            $insertar = "Insert into ventas_detalles 
				...
				id_forma_afectacion_tributaria_iva,descuento_prorrateado)
				values
                ...
                while (!$rsgrupos->EOF) {
                    ...
                    if ($idreceta > 0) {
                        ...
                        while (!$rsrecu->EOF) {
                            ...
                            $pp = intval($rsrecu->fields['idprod']);
                            $insu = intval($rsrecu->fields['idinsumo']);
                            $ca = floatval($rsrecu->fields['cantidad']) * $cantidad;
                            $co = floatval($rsrecu->fields['costo']);
                            $med = intval($rsrecu->fields['idmedida']);
                            $va = intval($rsrecu->fields['tipoiva']);
                            $mueve_stock_ins = trim($rsrecu->fields['mueve_stock_ins']);
        ...
        // no prorratear descuento, insertar como producto
        if ($prorratear_descuento != 'S') {
        ...
        $consulta = "
			...
			";
        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        // para los reportes de margen globales en informe ventas
        $consulta = "
			update ventas set 
			monto_iva = COALESCE(( select sum(subtotal_monto_iva) from ventas_detalles where ventas_detalles.idventa = ventas.idventa ),0),
        ...
        // verifica que tenga razon social
        $consulta = "select razon_social from ventas where idventa = $idventa;";
        $rsrz = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        // si no tiene asigna
        if (trim($rsrz->fields['razon_social']) == '') {
            $consulta = "
				update ventas set 
        ...
        // busca los registros del carrito y marca como registrado (todo lo que tiene cabecera y en cabecera esta como registrado y tiene idatc)
        $consulta = "
			select idventatmp 
			from tmp_ventares 
			where 
			...
			";
        $rsreg = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        while (!$rsreg->EOF) {
            $idventatmp = intval($rsreg->fields['idventatmp']);
            if ($idventatmp > 0) {
                $consulta = "
					update tmp_ventares set registrado = 'S' where idventatmp = $idventatmp
					";
                $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
            }
            $rsreg->MoveNext();
        ...
        // genera factura electronica si todo finalizo correcto
        if ($facturador_electronico == 'S') {
            if ($ticketofactura == 'FAC') {
        ...
        // Dark Kitchen
        if (intval($_SESSION['iddarkkitchen']) > 0) {
            $_SESSION['iddarkkitchen'] = 0;
            $_SESSION['iddarkkitchen'] = null;
            unset($_SESSION['iddarkkitchen']);
        }
        ...
        // pos bancard
        if ($idpostmp > 0) {
            // busca forma de pago de la venta
            $consulta = "
				SELECT gest_pagos_det.idpagodet, gest_pagos_det.idpago, gest_pagos_det.idformapago
				FROM `gest_pagos`
				inner join gest_pagos_det on gest_pagos.idpago = gest_pagos_det.idpago
				where 
				idventa = $idventa
				limit 1
				";
            $rsfpven = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
            $idformapago_det = $rsfpven->fields['idformapago'];
            $idpagodet = $rsfpven->fields['idpagodet'];
            // si es un pago del POS no de pantalla
            if ($idconfirmapago == 0) {
                // busca forma de pago del pedido
                $consulta = "
					select tarjetas_bancard.tipo as tipo_tarjeta, tarjetas_bancard.idformapago
					from tmp_ventares_cab_pos 
					left join tarjetas_bancard on tarjetas_bancard.issuerId = tmp_ventares_cab_pos.issuerId
					where 
					tmp_ventares_cab_pos.idtmpventares_cab = $idtmpventares_cab
					order by idtmpventares_cab_pos asc
					limit 1
					";
                $rsfpped = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                $tipo_tarjeta_pos = $rsfpped->fields['tipo_tarjeta'];
                $idformapago_pos = $rsfpped->fields['idformapago'];
                // si la forma de pago registrada es diferente a la del pos
                if ($idformapago_det != $idformapago_pos) {
                    if ($idformapago_pos > 0) {
                        $consulta = "
							update gest_pagos_det 
							set 
							idformapago = $idformapago_pos
							where
							idpagodet=$idpagodet
							";
                        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                        ...
                        $consulta = "
							update gest_pagos_det 
							set 
							idformapago = $idformapago_pos_sinidentificar
							where
							idpagodet=$idpagodet
							";
                        $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
            ...
            if ($idconfirmapago > 0) {
                $consulta = "
					...
					id = $idconfirmapago
					";
                $rstype = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
                // tarjeta credito
                if ($rstype->fields['account_type'] == 'TC') {
                    $consulta = "
						update gest_pagos_det 
						set 
						idformapago = 2
                ...
                // tarjeta debito
                if ($rstype->fields['account_type'] == 'TD') {
                    $consulta = "
						update gest_pagos_det 
						set 
						idformapago = 4
        ...
        // si hay integracion con pegasus
        $consulta = "
			select integracion_pegasus from sucursales where idsucu = $idsucursal
			";
        $rssuc = $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        if ($rssuc->fields['integracion_pegasus'] == 'S') {
            require_once("mesas/pegasus.php");
            $parametros_array = [
                'idventa' => $idventa
            ];
            $res = pedido_pegasus($parametros_array);
            if ($res['valido'] != 'S') {
                header("location: mesas/pegasus_error.php?id=" . $idventa);
                exit;
            }
        }


        // si el canal es mesa probablemente vino del modulo viejo
        if ($canal == 4) {
            // finaliza el atc
            $consulta = "
				update mesas_atc set estado = 3 where idmesa = $idmesa and estado <> 6 and estado <> 3
				";
            $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
            // libera la mesa
            $consulta = "
				update mesas set estado_mesa = 1 where idmesa = $idmesa
				";
            $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
        }
        ...
        // redir sin imprimir en modulo viejo
        if ($_POST['redir'] == 'S') {
