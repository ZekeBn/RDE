...
// Se agrega monto total de la orden
$consulta = "
select *,
...
(select proveedores.nombre from proveedores WHERE proveedores.idproveedor =  (Select compras_ordenes.idproveedor from compras_ordenes WHERE compras_ordenes.ocnum = embarque.ocnum)) as proveedor,
(Select compras_ordenes.idproveedor from compras_ordenes WHERE compras_ordenes.ocnum = embarque.ocnum) as idproveedor,

(select sum(compras_ordenes_detalles.precio_compra_total) from compras_ordenes_detalles 
where compras_ordenes_detalles.ocnum = (Select compras_ordenes.ocnum from compras_ordenes WHERE compras_ordenes.ocnum = embarque.ocnum) 
) as total

from embarque
where 
 estado = 1 
 and embarque.estado_embarque = 1
...
<script>
 if ($("#erroresEditarArticulo").is(':empty')) {
  ...
  function detalle_embarque(event,ocnum,idembarque){
	event.preventDefault();
	var parametros_array = {
	  ...
	  $.ajax({		  
		...
		success:  function (response) {
			$("#box_embarque_"+idembarque).css('display', 'contents');	
			$("#box_close_embarque_"+idembarque).css('display', 'table-row');	
			$("#embarque_"+idembarque).html(response);	
		},
    ...
    <table width="100%" class="table table-bordered jambo_table bulk_action">
	  <thead>
		<tr>
			...
			<th align="center">Total Compra</th>
		</tr>
	  </thead>
	  <tbody>
<?php while (!$rs->EOF) { ?>
		<tr>
			<td>
				...
				<div class="btn-group">
					<a href="javascript:void(0);" onclick="detalle_embarque(event,<?php echo $rs->fields['ocnum']; ?>,<?php echo $rs->fields['idembarque']; ?>)" class="btn btn-sm btn-default" title="Detalle" data-placement="right"  ><span class="fa fa-search"></span></a>
					<a href="tmpcompras_add.php?idproveedor=<?php echo $rs->fields['idproveedor']; ?>&ocnum=<?php echo $rs->fields['ocnum']; ?>&totaloc=<?php echo $rs->fields['total']; ?>" class="btn btn-sm btn-default" title="Select" data-toggle="tooltip" data-placement="right"  data-original-title="Select"><span class="fa fa-check"></span>Seleccionar</a>
			...
			<td align="center"><?php echo antixss(formatomoneda($rs->fields['total'], 3)); ?></td>
		</tr>
		<tr style="display:none;" id="box_close_embarque_<?php echo $rs->fields['idembarque']; ?>"><td colspan="7">
			<div  style="display:flex;justify-content:right;">
				<a href="javascript:void(0);" onclick="cerrar_embarque(event,<?php echo $rs->fields['idembarque']; ?>);" class="btn btn-sm btn-default">
								<span class="fa fa-close"></span>
							</a>
			</div>
					</td></tr>
		<tr style="display:none;" id="box_embarque_<?php echo $rs->fields['idembarque']; ?>">
		<td id="embarque_<?php echo $rs->fields['idembarque']; ?>" colspan="7"></td>
		</tr>
