...
$gscotizacion = floatval($gscotiz->fields['cotizacion']);
...
if ($tipo_cambio_factura < 1) {
    $tipo_cambio_factura = number_format($gscotizacion, 2);
}
...
while (!$rs_detalles->EOF) {
    ...
    if ($tipo_cambio_despacho_sin_formato !== null) {
        $total_fob = floatval($rs_detalles->fields['subtotal']) / $tipo_cambio_factura_sin_formato;

    } else {
        $total_fob = floatval($rs_detalles->fields['subtotal']) / $gscotizacion;
    }
    ...
    if ($usa_cot_despacho_factura == "S") {
        ...
        $gastos_totales_articulo_dolar = ($valor_subtotal_gastos_compra * $gasto_porcentaje) / $gscotizacion;
    ...
    if ($usa_cot_despacho_factura == "S") {
        ...
        $total_costo_cif_guarani = $total_cif * $gscotizacion;
    }
    ...
    $html .= "
                <tr>
                    ...
                    <td align=\"right\">$total_fob </td>
                    <td align=\"right\">$gasto_porcentaje </td>
                    <td align=\"right\">$gastos_totales_articulo_dolar </td>
                    <td align=\"right\">$costo_cif_anterior </td>
                    <td align=\"right\">$costo_cif</td>
                    <td align=\"right\">$total_cif</td>
                    <td align=\"right\">$diff_costo_cif</td>
                    <td align=\"right\">$costo_cif_guarani</td>
                    <td align=\"right\">$total_costo_cif_guarani</td>
...
if ($usa_cot_despacho_factura == "S") {
    ...
    $fobguarani = formatomoneda($total_fob_valor * floatval($gscotizacion), "0", "N");
    $cifguarani = formatomoneda($total_cif_valor * floatval($gscotizacion), "0", "N");
    $total_gastos_valor_dolar = $total_gastos_valor / floatval($gscotizacion);
    ...
    foreach ($facturas as $idmoneda => $monedas_array) {
        foreach ($monedas_array as $id_factura) {
            ...
            foreach ($id_factura as $factura) {
                ...
                } elseif (intval($factura->idcot) > 0 && $factura->usa_cot_despacho == "N") {
