...
// EJEMPLO DE QUERY CSV
// select date(`ventas`.`fecha`) as `fecha`,
// `ventas`.`factura`,
// `sucursales`.`nombre` as `sucursal`,
// REPLACE(
//     COALESCE(
//         (Select sum(`subtotal_costo`)-(sum(`subtotal_costo`)/11) as `subtotal` from `ventas_detalles`
//          where `idventa`=`ventas`.`idventa` and `iva`=10),0),'.',','
// 		) as `costo_grav_10`,
// REPLACE(
// 	COALESCE(
// 		(Select sum(`subtotal_costo`)-(sum(`subtotal_costo`)/21) as `subtotal` from `ventas_detalles` where `idventa`=`ventas`.`idventa` and `iva`=5),0
// 		),'.',','
// 		) as `costo_grav_05`,
// REPLACE(
// 	COALESCE(
// 		(Select sum(`subtotal_costo`) as `subtotal` from `ventas_detalles` where `idventa`=`ventas`.`idventa` and `iva`=0),0
// 	),'.',',') as `costo_exenta`,
// REPLACE(
// 	COALESCE(
...
// where `ventas`.`estado` <> 6 and date(`ventas`.`fecha`) >= '2023-05-01'
// and date(`ventas`.`fecha`) <= '2023-05-02'
