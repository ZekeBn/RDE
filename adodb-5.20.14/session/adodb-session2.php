    ...
    */
    public static function write($key, $oval)
    {
        ...
        if ($crc !== '00' && $crc !== false && $crc == (strlen($oval) . crc32($oval))) {
            ...
            $rs = $conn->Execute($sql, [$expirevar,$key]);
            return true;
        }
        ...
        if (!$clob) {	// no lobs, simply use replace()
            $rs = $conn->Execute("SELECT COUNT(*) AS cnt FROM $table WHERE $binary sesskey = ".$conn->Param(0), [$key]);
            if ($rs) {
                $rs->Close();
            }
            ...
            $rs = $conn->Execute($sql, [$val,$expireref,$key]);
            ...
            $rs = $conn->Execute($sql, [$expireref,$key]);
    ...
    public static function gc($maxlifetime)
    {
        ...
        if ($rs) {
            ...
            while (!$rs->EOF) {
                ...
                $del = $conn->Execute("DELETE FROM $table WHERE sesskey=".$conn->Param('0'), [$key]);
                $rs->MoveNext();
                $ccnt += 1;
