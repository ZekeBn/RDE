...
ef")."</p>";//'
  ...
  KEY `myindex` (`row1`,`row2`)
            ...
            case 'oci8':
            case 'oci805':
                $arr = ['first' => 'Caroline','last' => 'Miranda'];
                $amt = rand() % 100;
    ...
    if ($db->dataProvider == 'oci8') {
        $array = ['zid' => 1,'zdate' => date('Y-m-d', time())];
    } else {
        $array = [1,date('Y-m-d', time())];
    }
    ...
    for ($loop = 0; $loop < 1; $loop++) {
        ...
        $rs = $db->CacheExecute(4, "select distinct firstname,lastname from ADOXYZ");
        if ($rs) {
            print ' Multiple, Alan and George selected: '. $rs->GetMenu('menu', ['Alan','George'], false, true);
            if (empty($rs->connection)) {
                print "<b>Connection object missing from recordset</b></br>";
            }
        } else {
    ...
    $ret = $db->Replace(
        'ADOXYZ',
        ['id' => 1,'firstname' => 'Caroline','lastname' => 'Miranda'],
        ['id'],
        $autoq = true
    );
    if (!$ret) {
    ...
    $ret = $db->Replace(
        'ADOXYZ',
        ['id' => 1000,'firstname' => 'Harun','lastname' => 'Al-Rashid'],
        ['id','firstname'],
        $autoq = true
    );
    if ($ret != 2) {
    ...
    $ret = $db->Replace(
        'ADOXYZ',
        ['id' => 1000,'firstname' => 'Sherazade','lastname' => 'Al-Rashid'],
        'id',
        $autoq = true
    );
    if ($ret != 1) {
