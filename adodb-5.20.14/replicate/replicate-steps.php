...
if ($BA) {
    ...
    if (1) {
        $STP_BA = " and s_stagecat in (select stg_stagecat from kbstage where stg_bacode='$BA')";
    } # OLDER KBSTEP
    else {
        $STP_BA = " and s_bacode='$BA'";
    }  # LATEST KBSTEP format
...
$rep->ReplicateData('KBSTAGE', 'KBSTAGE', [], " where (1=1)$STG_BA$STG_STG");
$rep->ReplicateData('KBSTEP', 'KBSTEP', [], " where (1=1)$STP_BA$STP_STG");
$rep->ReplicateData('KBQTYPE', 'KBQTYPE', [], " where qu_mode in ('STAGE','STEP')$QTY_BA$QTY_STG");
