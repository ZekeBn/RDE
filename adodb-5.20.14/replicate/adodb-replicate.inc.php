...
class ADODB_Replicate
{
    ...
    public function __construct($connSrc, $connDest, $connSrc2 = false, $connDest2 = false)
    {
        ...
        foreach ($types as $name => $t) {
            ...
            if ($this->copyTableDefaults) {
                if (isset($t->default_value)) {
                    $v = $t->default_value;
                    if ($mt == 'C' or $mt == 'X') {
                        $v = $this->connDest->qstr($v);
                    } // might not work as this could be function
                    $s .= ' DEFAULT '.$v;
                }
            }
...
";
    ...
    */
    ...
    public function Merge(
        ...
        if ($set) {
            $this->updateSrcFn = [implode(', ', $set)];
        } else {
            $this->updateSrcFn = '';
        }


        $extra[$srcCopyFlagFld] = [$dest->qstr($flagvals[0]),$dest->qstr($flagvals[$copyDoneFlagIdx])];
    ...
    */
    ...
    public function _doerr($reason, $selflds)
    {
        $fn = $this->errHandler;
        if ($fn) {
            $fn($this, $reason, $selflds);
        } // set $this->neverAbort to true or false as required inside $fn
    }
}
