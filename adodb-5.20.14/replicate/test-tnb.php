...
function MergeDataJohnTest($rep, $table, $pkey)
{
    ...
    if (strpos($rep->connDest->databaseType, 'mssql') !== false) {  # oracle ==> mssql
        ...
        $pkeyarr = [[$pkey],false,['MSSQL_ID']];# array('MSSQL_ID', 'ORA_ID'));
    } else {  # mssql ==> oracle
        $ignoreflds = [$pkey];
        $ignoreflds[] = 'ORA_ID';
        $set = '';
        #$set = 'ORA_ID=isnull($INSERT_ID,ORA_ID)';
        $pkeyarr = [[$pkey],['MSSQL_ID']];
    }
    $rep->execute = true;
    #$rep->updateFirst = false;
    $ok = $rep->Merge($table, $dtable, $pkeyarr, $ignoreflds, $set, 'UpdatedOn', 'CopiedFlag', ['Y','N','P','='], 'CopyDate');
    var_dump($ok);
