...
AND    b.name = 'sorts (memory)'",
    ...
    'data cache size' => ['DATAC',
        ...
        'dynamic memory usage' => ['CACHE', "select '-' from dual", '=DynMemoryUsage'],
...
having count(*) > 100)",'These are sql statements that should be using bind variables'),*/
        'index cache cost' => ['COST',
            "select value from v\$parameter where name = 'optimizer_index_caching'",
            '=WarnIndexCost'],
        'random page cost' => ['COST',
            "select value from v\$parameter where name = 'optimizer_index_cost_adj'",
            '=WarnPageCost'],
    'Waits',
        'Recent wait events' => ['WAITS','select \'Top 5 events\' from dual','=TopRecentWaits'],
//		'Historical wait SQL' => array('WAITS','select \'Last 2 days\' from dual','=TopHistoricalWaits'), -- requires AWR license
    'Backup',
        'Achivelog Mode' => ['BACKUP', 'select log_mode from v$database', '=LogMode'],

        'DBID' => ['BACKUP','select dbid from v$database','Primary key of database, used for recovery with an RMAN Recovery Catalog'],
        'Archive Log Dest' => ['BACKUP', "SELECT NVL(v1.value,v2.value)
FROM v\$parameter v1, v\$parameter v2 WHERE v1.name='log_archive_dest' AND v2.name='log_archive_dest_10'", ''],

        'Flashback Area' => ['BACKUP', "select nvl(value,'Flashback Area not used') from v\$parameter where name=lower('DB_RECOVERY_FILE_DEST')", 'Flashback area is a folder where all backup data and logs can be stored and managed by Oracle. If Error: message displayed, then it is not in use.'],

        'Flashback Usage' => ['BACKUP', "select nvl('-','Flashback Area not used') from v\$parameter where name=lower('DB_RECOVERY_FILE_DEST')", '=FlashUsage', 'Flashback area usage.'],

        'Control File Keep Time' => ['BACKUP', "select value from v\$parameter where name='control_file_record_keep_time'",'No of days to keep RMAN info in control file.  Recommended set to x2 or x3 times the frequency of your full backup.'],
        'Recent RMAN Jobs' => ['BACKUP', "select '-' from dual", "=RMAN"],
      ...
      'Storage', 'Tablespaces' => ['TABLESPACE', "select '-' from dual", "=TableSpace"],
        false
