...
/*if(intval($idmesa) == 0){
    echo "Codigo QR invalido, favor volver a escanear.";
    exit;
}*/
...
if (isset($_POST['MM_mesas_qr']) && $_POST['MM_mesas_qr'] == 'form1') {
    ...
    if ($pin_form == $pin_atc) {
        ...
        $_SESSION['id_mesa'] = $id_mesa;
        $_SESSION['idatc'] = $idatc;
        header("location: ./dashboard_bootstrap.php");
        exit;
    } else {
        $errores_login .= "Pin incorrecto, favor introduzca nuevamente o solicite al mozo el reseteo.<br />";
    }
}
...
$consulta = "
select empresa from empresas where idempresa = 1
";
...
<div class="min-h-screen bg-gray-100 text-gray-900 flex justify-center">
    <div class="w-screen h-screen flex justify-center flex-1">
        ...
        <div class="relative px-4 sm:px-6 lg:px-8 max-w-lg mx-auto">
                <img class="rounded-t shadow-lg resized-image"  style="height:168px;" src="fondo.png<?php //echo $image;?>"  alt="Pay background" />
            </div>
            <div class="relative px-4 sm:px-6 lg:px-8 pb-8 max-w-lg mx-auto" >
                <div class="bg-white px-8 pb-6 rounded-b shadow-lg">
                    ...
                    <div class="text-center mb-6">
                        <div class="mb-2">
                            <img class="-mt-8 inline-flex rounded-full" src="../gfx/empresas/emp_1.png<?php //echo $logo;?>" width="120" height="120" alt="User" />
                        </div>
                        <h1 class="text-xl leading-snug text-gray-800 font-semibold mb-2">¡Bienvenido a <?php echo antixss($rs->fields['empresa']); ?>!</h1>
<?php if (intval($id_mesa) > 0) { ?>
						<strong>Mesa: <?php echo antixss($rsmesa->fields['numero_mesa']); ?></strong><br />

<?php if (trim($errores) != "") { ?>
<div class="text-sm"><?php echo $errores; ?></div>
<?php } ?>
<?php if (trim($errores) == "") { ?>
<div class="text-sm">
 Para acceder a las funciones de la mesa simplemente introduce tu pin y haz click en ingresar.<br />
Si aun no tienes pin puedes solicitar al mozo que lo genere para ti.
</div>
<?php } ?>
                    </div>
...
<?php if (intval($id_mesa) > 0) { ?>
                            <div class="space-y-4">
                                <!-- Card Number -->
                                <div>
                                    <label class="block text-sm font-medium mb-1" for="pin">PIN <span class="text-red-500">*</span></label>
...
<div class="alert alert-danger alert-dismissible fade in text-sm" role="alert" style="color:red;">
<?php echo $errores_login; ?>
</div>
<?php } ?>
                            </div>
                            <!-- Form footer -->
...
<?php } else { // if(intval($id_mesa) > 0){?>
<br />
Lo sentimos, ha ocurrido un error.<br />
Por favor, escanee nuevamente el código QR.
<br />
<?php } // if(intval($id_mesa) > 0){?>
                </div>
            </div>
...
<div style="text-align:center;">Powered by: <br /><a href="https://restaurante.com.py/" target="_blank"><img src="../img/logo_carrito.png" style="height:30px; margin:0px auto;" height="30px;" /></a></div>
<?php } // if(intval($id_mesa) > 0){?>
