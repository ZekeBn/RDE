...
while (!$rsd->EOF) {
    ...
    if ($sin_ubicar > 0) {
        if ($lote != "NULL") {
            $resultados2 .= "
            <option href='javascript:void(0);' data-hidden-value=$categoria data-hidden-codorigen=$codigo_origen onclick=\"este_producto({vencimiento: '$vencimiento', lote: $lote,idinsumo: $idinsumo,maneja_lote: $maneja_lote, descripcion: $descripcion, medida:$medida, medida2: $medida2, medida3:$medida3, cant_medida2: $cant_medida2, cant_medida3: $cant_medida3, id_cajas_edi: $id_cajas_edi, idmedida2: $idmedida2, idmedida3: $idmedida3, idmedida: $idmedida, cant_caja_edi: $cant_caja_edi, iddeposito: $iddeposito, sin_ubicar: $sin_ubicar, idregseriedptostk: $idregseriedptostk });\">[$idinsumo]-$descripcion L:$lote V:'$vencimiento_format '($medida)</option>
            ";
        } else {
            $clase = "";
            if ($i % 2 == 1) {
            ...
            $resultados2 .= "
            <option href='javascript:void(0);' $clase data-hidden-value=$categoria data-hidden-codorigen=$codigo_origen onclick=\"este_producto({vencimiento: '$vencimiento', lote: $lote,idinsumo: $idinsumo,maneja_lote: $maneja_lote, descripcion: $descripcion, medida:$medida, medida2: $medida2, medida3:$medida3, cant_medida2: $cant_medida2, cant_medida3: $cant_medida3, id_cajas_edi: $id_cajas_edi, idmedida2: $idmedida2, idmedida3: $idmedida3, idmedida: $idmedida, cant_caja_edi: $cant_caja_edi, iddeposito: $iddeposito, sin_ubicar: $sin_ubicar, idregseriedptostk: $idregseriedptostk });\">[$idinsumo]-$descripcion ($medida)</option>
            ";
...
<script>
    window.onload = function(){
        var div_codbar = $("#box_codbar_gen_image").text();
        div_codbar = div_codbar.trim();
        if (div_codbar === "") {
            $("#box_codbar_gen_image").css("display", "none");
        }
    }
    ...
    function este_producto(parametros){
        ...
        // I:3;L:2;V:2023-05-05
        var lote = parametros.lote ==0 ? "": parametros.lote;
        var codbar = "I:"+parametros.idinsumo+";L:"+lote+";V:"+parametros.vencimiento;
            var parametros2 = {
                    "codbar"   : codbar
            };
            $.ajax({
                    data:  parametros2,
                    url:   'codbar_gen_image.php',
                    type:  'post',
                    beforeSend: function () {
                    ...
                    success:  function (response) {
                        $("#box_codbar_gen_image").html(response);
                        // var codbar = $("#box_codbar_gen_image").text();
                        // codbar = codbar.trim();
                        // if (codbar === "") {
                        //     $("#box_codbar_gen_image").css("display", "flex !important");
                        // }
                    }
            });
...
<div class="col-md-6" id="box_codbar_gen_image">
    <?php require_once("codbar_gen_image.php"); ?>
</div>
