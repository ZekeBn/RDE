...
<script>
	...
	function conteo_guarda_tmp(id){
		...
		var lote = $("#lote_"+id).val();
		var vencimiento = $("#vencimiento_"+id).val();
		var medidaSelected = $("#medida_select_"+id).find("option:selected");
		var idmedida = medidaSelected.val();
		var orden = medidaSelected.data("hidden-orden");
		var cantidad_por_medida = medidaSelected.data("hidden-value");
		...
		if(orden != 0 ){
			cantidad = cantidad_por_medida*cantidad;
		}
			...
			$.ajax({
						data:  parametros,
						url:   'conteo_stock_tmp_pdv.php',
						type:  'post',
						beforeSend: function () {
							$("#prod_tmp").html('Guardando...');
						},
						success:  function (response) {
							$("#prod_tmp").html(response);
							// sumartodo();
						}
			});	
		...
		<div >
    		<table width="70" border="0" >
				<tbody>
					<tr>
					<td width="62">
						<a href="conteo_stock_pdv.php" class="btn btn-sm btn-default"><span class="fa fa-reply"></span> Volver</a>
					</td>
					</tr>
				</tbody>
			</table>
...
<div id="prod_tmp">
	<?php require_once("conteo_stock_tmp_pdv.php"); ?>
</div>
...
<table width="900" border="0">
  <tbody>
    <tr>
      <td align="center"><input type="submit" name="submit" id="submit_1" value="Guardar" class="btn btn-lg btn-default" onmouseup="accionbtn(1);" /></td>
      <?php if ($afectastock_conteo_pdv == 'S') { ?>
      <td align="center"><input type="submit" name="submit_" id="submit_3" value="Finalizar y Afectar Stock" class="btn btn-lg btn-default" onmouseup="accionbtn(3);" /></td>
	  <?php } ?>
      </tr>
  </tbody>
</table><br /><br />
