        ...
        if ($valido == 'S') {
            ...
            foreach ($array_res as $fila) {
                ...
                if ($i > 1) {
                    ...
                    $razon_social = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[1]), 0, 200)), "text");
                    $ruc = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[2]), 0, 45)), "text");
                    $documento = antisqlinyeccion(limpia_csv_externo(trim($fila[3])), "int");
                    $sucursal = antisqlinyeccion(limpia_csv_externo(trim($fila[4])), "text");
                    $telefono = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[5]), 0, 45)), "text");
                    $direccion = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[6]), 0, 150)), "text");
                    $comentario = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[7]), 0, 500)), "text");
                    $linea_credito = antisqlinyeccion(floatval(str_replace(',', '.', $fila[8])), "float");
                    $nombre = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[9]), 0, 45)), "text");
                    $apellido = antisqlinyeccion(limpia_csv_externo(substr(trim($fila[10]), 0, 45)), "text");
                    $tipo_cliente = antisqlinyeccion(substr(trim($fila[11]), 0, 45), "text");
                    $latitud = antisqlinyeccion(floatval(trim($fila[12])), "float");
                    $longitud = antisqlinyeccion(floatval(trim($fila[13])), "float");
                    ...
                    $consulta = "
					insert into cliente_import
					(idclienteimpcab, razon_social, ruc, documento, telefono, direccion, comentario, linea_credito, nombre, apellido, sucursal, 
					tipo_cliente, latitud, longitud, email)
					values
					($idclienteimpcab,$razon_social, $ruc, $documento, $telefono, $direccion, $comentario, $linea_credito, $nombre, $apellido, $sucursal, $tipo_cliente, $latitud, $longitud, $email)
					";
                    //echo $consulta;exit;
                    $conexion->Execute($consulta) or die(errorpg($conexion, $consulta));
