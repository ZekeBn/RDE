    ...
    if (strpos(($col->plaintext), "del mes de $nombreMes $anho")) {
        foreach (($col->parentNode()->parentNode()->parentNode()->children()[1]->find("table tr")) as $td) {
            if ($td->find('td', 0)->plaintext == $dia_ayer) {
                ...
                $cotizacion = $td->find('td', 1)->plaintext;
                break;
            }
        }
    }
...
if (!is_numeric($cotizacion) || $cotizacion == 0) {
    $valido = "N";
    $alerta = "S";
    $errores .= "Comuníquese con el soporte técnico, ya que www.set.gov.py/web/portal-institucional/cotizaciones puede estar fuera de servicio o alterado estructuralmente";
}
...
if ($valido == "S") {
    ...
    $consulta = "
        ...
        (cotizacion, estado, fecha, tipo_moneda, registrado_el, registrado_por)
        values
        ($cotizacion, $estado, '$fecha', $tipo_moneda, $registrado_el, $registrado_por)
        ";
